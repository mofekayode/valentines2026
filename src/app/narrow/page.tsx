...
export default function NarrowPicker() {
  ...
  useEffect(() => {
    // Load photos from public/media via API
    fetch('/api/media')
      .then(res => res.json())
      .then(data => {
  ...
  const handleSelect = useCallback((approved: boolean) => {
    if (!photos[currentIndex]) return;
    const photo = photos[currentIndex];

    if (approved) {
      setSelected(prev => [...prev, photo.filename]);
    } else {
      setRejected(prev => [...prev, photo.filename]);
    }
  ...
  }, [isDone, handleSelect, handleUndo]);
  ...
  if (photos.length === 0) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <p className="text-white">No photos found in public/media.</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-8">
        ...
        <div className="flex gap-4">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="px-8 py-3 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-full font-medium hover:opacity-90 transition disabled:opacity-50"
          >
            {isSaving ? 'Saving...' : 'Save Final Selection'}
          </button>
          <button
            onClick={() => {
              setCurrentIndex(0);
        ...
        {selected.length > 0 && (
          <div className="mt-12 w-full max-w-4xl">
            ...
            <div className="grid grid-cols-4 md:grid-cols-6 gap-2">
              {selected.slice(0, 24).map((filename, i) => (
              ...
              {selected.length > 24 && (
                <div className="aspect-square bg-white/10 rounded-lg flex items-center justify-center">
                  <span className="text-white/40">+{selected.length - 24} more</span>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="p-4 text-center">
        <h1 className="text-xl text-white font-light">Narrow Down Your Selection</h1>
        <p className="text-white/40 text-sm">{currentIndex + 1} of {photos.length} • {selected.length} kept</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="relative w-full max-w-lg aspect-[3/4] rounded-3xl overflow-hidden shadow-2xl"
          >
            {currentPhoto && (
              <Image
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
              <p className="text-white/60 text-sm">{currentPhoto?.filename}</p>
            </div>
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <p className="text-center text-white/20 text-xs pb-4">
        Keyboard: ← Reject • → or Space Accept • Backspace Undo
      </p>
    </div>
  );
}
